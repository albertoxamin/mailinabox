...
def do_dns_update(env, force=False):
	...
	# Rspamd signs all domains with a single shared key (see
	# setup/rspamd.sh, /etc/rspamd/local.d/dkim_signing.conf), so no
	# per-domain table needs to be written when domains are added.
...
def build_zone(domain, domain_properties, additional_records, env, is_zone=True):
	...
	if domain_properties[domain]["mail"]:
		...
		# Append the DKIM TXT record to the zone. The key file is the one
		# generated by setup/rspamd.sh (or imported from a prior OpenDKIM
		# install); the same key signs all domains. Skip if the user has
		# set a DKIM record already.
		dkim_record_file = os.path.join(env['STORAGE_ROOT'], 'mail/dkim/mail.txt')
